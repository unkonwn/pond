...
-(void)setUPAllChildViewController{
    ...
    [itemsArray enumerateObjectsUsingBlock:^(NSDictionary *dict, NSUInteger idx, BOOL *stop) {
        UIStoryboard *stroyB = [UIStoryboard storyboardWithName:dict[kClassKey]
                                                         bundle:nil];
        UINavigationController *nvc = [stroyB instantiateInitialViewController];
        
        nvc.viewControllers[0].title = dict[kTitleKey];
        ...
        item.font = self.font;
        //        NSLog(@"item");
        [self addChildViewController:nvc];//添加视图控制器
